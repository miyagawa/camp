...
sub parse_options {
    ...
    if ($0 ne '-' && !-t STDIN){ # e.g. $ camp < author/requires.camp
...
sub setup_home {
    ...
    unless (_writable($self->{home})) {
        die "Can't write to camp home '$self->{home}': You should fix it with chown/chmod first.\n";
    }
    ...
    print $out "camp $Camp::VERSION on perl $] built for $Config{archname}\n";
...
|   - run me with --local-lib option e.g. camp --local-lib=~/perl5
!   - Set PERL_CAMP_OPT="--local-lib=~/perl5" environment variable (in your shell rc file)
...
sub setup_local_lib {
    ...
    require local::lib;
    {
        local $0 = 'camp'; # so curl/wget | perl works
        $base ||= "~/perl5";
...
sub configure {
    ...
    local $ENV{PERL5_CAMP_IS_RUNNING} = $$;
